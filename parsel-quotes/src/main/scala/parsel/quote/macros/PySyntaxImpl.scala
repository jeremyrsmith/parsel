...
import parsel.parse.{ParseError, Parser}
  ...
  def lintSplice(splices: c.Tree*): Tree = c.prefix.tree match {
    case Apply(_, List(Apply(_, stringPartsTrees))) =>
      ...
      val stableSplices = splices.collect {
        case tree@Ident(_) if !(tree.tpe.dealias <:< weakTypeOf[parsel.ast.Tree]) && tree.symbol.isTerm && tree.symbol.asTerm.isStable =>
          (tree.symbol, quotableInfo(tree))
      }.reverse.toMap

      val quotes = splices.collect {
        case tree if stableSplices.contains(tree.symbol) => stableSplices(tree.symbol)
        case tree if !(tree.tpe.dealias <:< weakTypeOf[parsel.ast.Tree]) => quotableInfo(tree)
      }
